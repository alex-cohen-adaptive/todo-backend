...
router
    .post("/todos",authenticationService.verifyToken, async (req, res, next) => {
        console.log(req.body);
        res.sendStatus(201).send( await todoService.create(req.body));
        next();
        // return next();
    })
    .put("/todos",authenticationService.verifyToken,async (req, res, next) => {
    ...
    .get("/todos",authenticationService.verifyToken, async (req, res, next) => {
        let page:number = parseInt(req.query.page as string)
        let size:number = parseInt(req.query.size as string);
        res.send(await todoService.getAll(new Page(page,size)));
        next();
    })
    .delete("/todos/:id" ,authenticationService.verifyToken,async (req,res,next) => {
